...
export function importFromJson (fileStr) {
  ...
  if (obj['@context'] !== 'https://atek.cloud/adb-export') {
    throw new Error('Unable to import file: must be a JSON-LD file with a @context of "https://atek.cloud/adb-export')
  }
  const records = []
...
export function exportAsJson (records) {
  const exportJson = {
    "@context": "https://atek.cloud/adb-export"
  }
